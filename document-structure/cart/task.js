// Функция добавления обработчика события click на все элементы с классом elementClassName
...
//#region Change quantity functions
...
const addProductToCart = e => {
  ...
  let delay = 100;
  let duration = 800;
  // Функция анимации полета картинки в точку с left и top
  const flyImageTo = (left, top) => {
    let flyingImg = document.createElement("img"); // создаем элемент <img>
    ...
    document.querySelector("body").appendChild(flyingImg); // добавляем <img> в <body>
    ...
    setTimeout(() => {
      ...
      `; // задаем конечные координаты, закон и время анимации
    }, delay);
    setTimeout(() => {
      flyingImg.remove();
    }, duration + delay); // удаляем <img> из <body>
  };
